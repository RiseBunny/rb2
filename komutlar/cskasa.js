...
exports.run = (client, message) => {
  const lang = getLangSync(message.author.id);
    ...
    .setFooter({ text: (lang === "en" ? `Case opened by (${message.author.username}) | Case Price: 80$ | Items from the case are not kept, this command is just for fun` : `Kasayı açan (${message.author.username}) | Kasa Fiyatı: 80$ | Kasadan çıkan bütün ürünler sizde kalmaz sadece eğlence için yapılmış bir komutdur`) })

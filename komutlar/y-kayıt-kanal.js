...
exports.run = async (client, message, args) => {
 ...
 if (!message.member.permissions.has(Discord.PermissionFlagsBits.Administrator)) {
  ...
  if (!message.mentions.channels.first()) return message.channel.send((lang === "en" ? `⚠️ To Set This Feature Mention A Channel Example: !register-channel #register` : `⚠️ Bu Özelliği Ayarlamam İçin Bir Kanal Etiketlemelisin Örnek: !kayıt-kanal #kayitkanal`));
  ...
  const successEmbed = new Discord.EmbedBuilder()
    .setColor('Blue')
    .setTitle((lang === "en" ? 'Channel Set Successfully!' : 'Kanal Başarıyla Ayarlandı!'))
    .setFooter({ text: (lang === "en" ? 'Mention again to change the channel.' : 'Kanalı değiştirmek istersen tekrar etiketlemelisin.') })
    .setDescription((lang === "en" ? `» Channel set to ${channel}!` : `» Kanal: ${channel} olarak ayarlandı!`));
